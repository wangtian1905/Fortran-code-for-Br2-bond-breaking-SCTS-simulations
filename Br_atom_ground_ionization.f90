!  Github_Br2_SCTS_codes_full_for_Nat_Comm.f90 
!
!  FUNCTIONS:
!  Github_Br2_SCTS_codes_full_for_Nat_Comm - Entry point of console application.
!

!****************************************************************************
!
!  PROGRAM: Github_Br2_SCTS_codes_full_for_Nat_Comm
!
...
implicit none
!-----------------------------------------------------------------------------------------------
integer,parameter::gn=500000000!500000000!500000000!10000000!500000000!20000 0000!1000000000!1000000000!1000000000!500000000!<the size of the ensemble>!old_value:100000000
...
!real*8,parameter::Ip=0.3865!0.457!<the ionization potential of pi_u(3/2) Bromine molecule><a.u.>!0.457 is for pi_u MO,while 0.434 is for sigma_u MO!
!real*8,parameter::Ip_ph_ph=0.3865!0.457!0.457!0.434!0.457!<the ionization potential for phase integration!> 
integer,parameter::N_orb=4!<number of molecular orbitals included for ionization>
real*8,parameter::dice_orb_wth=1.d0/N_orb!<dice width of orbital label random grabing>
real*8,parameter::Ip1=0.434!0.434!<the ionization potential of pi_u orbital><a.u.>
real*8,parameter::Ip2=0.434!0.434!0.38655!0.38655!<the ionization potential of pi_g orbital><a.u.>
!real*8,parameter::Ip3=0.457!0.27262!0.38655!<the ionization potential of sigma_u orbital><a.u.>
!real*8,parameter::Ip4=0.457!0.536457!0.38655!<the ionization potential of sigma_g orbital><a.u.>
real*8,parameter::orb_prb_chk1=0.0d0
real*8,parameter::orb_prb_chk2=0.5d0
real*8,parameter::orb_prb_chk3=1.0d0
!real*8,parameter::Ip3=0.27225!<the ionization potential of sigma_u orbital><a.u.>
integer,parameter::lgth_orb_dis=5
real*8,parameter::Z_N=35
real*8,parameter::e_N=34.5
real*8,parameter::eta0=4.369
real*8,parameter::eta1=1.686!<eta1 X10>
real*8,parameter::xi0=1.407
real*8,parameter::xi1=1.897!<xi1 X10>
real*8,parameter::eta=eta0+eta1*(Z_N-e_N)/10
real*8,parameter::xi=xi0+xi1*(Z_N-e_N)/10
...
!real*8,parameter::R0=4.32!<internuclear distance of Bromine molecule><a.u.>
...
real*8,parameter::n_R_Atm=0.0!0.0!4.3!4.3!<internucleus distance of ground Br2 molecule; for Br case, set zero!><a.u.>
real*8,parameter::n_R_Mlc=0.0!0.0<internucleus distance of ground Br2 molecule; for Br case, set zero!><a.u.>
!real*8,parameter::MO_intgrlx=2.2326 !<integral of molecular orbital in full momentum space for normalization><a.u.>
!real*8,parameter::MO_ph_pos=0.5*pi !<phase of the MO lobe which has positive transverse momentum>
!real*8,parameter::MO_ph_neg=-0.5*pi !<phase of the MO lobe which has negative transverse momentum>
!real*8,parameter::MO_nde_0=0.0
!real*8,parameter::MO_nde_pos_u_1=0.356
!real*8,parameter::MO_nde_pos_u_2=1.08
!real*8,parameter::MO_nde_neg_u_1=-0.356
!real*8,parameter::MO_nde_neg_u_2=-1.08
!real*8,parameter::MO_nde_pos_g_1=0.726
!real*8,parameter::MO_nde_pos_g_2=1.44
!real*8,parameter::MO_nde_neg_g_1=-0.726
!real*8,parameter::MO_nde_neg_g_2=-1.44
!real*8,parameter::thta=(thta_dg/180)*pi!<alignment angle of molecule><radian>
real*8,parameter::apha_N=0.0!can be set within small range: 0.0~2.0!<polarizability of atom for considering stark shifted potential in initial state preparation><a.u.><This value represents the polarizability difference!>
...
real*8,parameter::apha_II_Atm=12!<atomic polarizability contribution for BBS ionization dynamics><a.u.>
real*8,parameter::apha_II_Mlc=12!<molecular polarizability contribution for BBS ionization dynamics><a.u.>
...
real*8::M_cos(1:len_Y,1:len_X),M_sin(1:len_Y,1:len_X),Spec_grdX(1:len_X),Spec_grdY(1:len_Y)
!real*8::MO_pxy(1:len_MO_Y,1:len_MO_X),MO_grdX(1:len_MO_X),MO_grdY(1:len_MO_Y),MO_per(1:len_MO_per)
...
real*8::n_Z1,n_Z2,n_R,apha_II
 integer::orb_labl_dis(1:lgth_orb_dis)
 real*8::orb_dis_lbl!,orb_prb
 real*8::orb_prb(1:N_orb),chk_pnt(1:(N_orb+1))
!real*8::py_0(1:gn),py_A(1:py_n_v),py_sta(1:py_n_v),py_sta_n(1:py_n_v)
!sta_ana_1D_number(A_exp,length_exp,n_v,v_lbound,v_ubound,A_sta,C_sta,C_sta_norm)
!-----------------------------------------------------------------------------------------------
!real*8::M_cos(1:len_Y,1:len_X),M_sin(1:len_Y,1:len_X),Spec_grdX(1:len_X),Spec_grdY(1:len_Y)
!-----------------------------------------------------------------------------------------------
...
!do i=1,p_per_div_n
!read(24,*) p_per_grid(i),p_per_wght(i)
!end do
write(24,*) 'Program starts!'

!$OMP PARALLEL NUM_THREADS(n_threads)  DEFAULT(private) SHARED(M_cos,M_sin,p_per_wght1,p_per_wght2,orb_labl_dis,chk_pnt)
...
!do i=1,N_orb
!    if((dice_orb<chk_pnt(i+1)).and.(dice_orb>chk_pnt(i)))then
!        orb_labl=i
!    end if
!end do
!orb_labl=int(dice_orb/dice_orb_wth)+1
!write(27,*)dice_orb,orb_labl
call sta_ana_1D_wght_lbl_dis(orb_labl,lgth_orb_dis,orb_labl_dis)
...
pper_wght1=p_per_wght1(p_per_i+1)

!if((pper>MO_nde_0).and.(pper<MO_nde_pos_u_1))then
!    MO_ph=MO_ph_neg
!else if((pper>MO_nde_pos_u_1).and.(pper<MO_nde_pos_u_2))then
!    MO_ph=MO_ph_pos
!else if((pper<MO_nde_0).and.(pper>MO_nde_neg_u_1))then
!    MO_ph=MO_ph_pos
!else if((pper<MO_nde_neg_u_1).and.(pper>MO_nde_neg_u_2))then
!    MO_ph=MO_ph_neg
!end if
    ...
    n_R=n_R_Atm
    apha_II=apha_II_Atm
pper_wght1=p_per_wght2(p_per_i+1)

!if((pper>MO_nde_0).and.(pper<MO_nde_pos_g_1))then
!    MO_ph=MO_ph_pos
!else if((pper>MO_nde_pos_g_1).and.(pper<MO_nde_pos_g_2))then
!    MO_ph=MO_ph_neg
!else if((pper<MO_nde_0).and.(pper>MO_nde_neg_g_1))then
!    MO_ph=MO_ph_pos
!else if((pper<MO_nde_neg_g_1).and.(pper>MO_nde_neg_g_2))then
!    MO_ph=MO_ph_neg
!end if
end if
MO_ph=0.d0
!Linear interpolation for calculating the weight of initial transverse momentum in MO
!p_per_i=int((pper-p_per_lb)/p_per_dlt)
!p_per1=p_per_grid(p_per_i+1)
!p_per2=p_per_grid(p_per_i+2)
!pper_wght1=p_per_wght(p_per_i+1)
!pper_wght2=p_per_wght(p_per_i+2)
!MO_wght=pper_wght1+(pper-p_per1)*(pper_wght2-pper_wght1)/(p_per2-p_per1)!(there is a problem on (p_per2-p_per1)!)
!Linear interpolation end
wght=tnl_wght(Ip,abs(Ex0),pper)!*pper_wght1
!write(24,*) py,wght
...
t=tnlt0
 S_phx=0.d0 
S_phx0=H_ph(px,py,rx,ry,n_Z1,n_Z2,n_R,apha_II)!H_ph(px,py,rx,ry)!H_ph(px,py,rx,ry,n_Z1,n_Z2,n_R,apha_II)
ion_jdg=0
h=h1
...
call sym_adp_algrthm(px,py,rx,ry,t,h,n_Z1,n_Z2,n_R,apha_II)!sym_adp_algrthm(px,py,rx,ry,t,h,n_Z1,n_Z2,n_R,apha_II)
   ...
   S_phx0=S_phx1
    ! integral end***
   !elec_egy0=elec_egy(px,py,rx,ry)
   ...
   !if((elec_egy0>Ip2).and.(elec_rr<r_impct))then
   !    good_trj=0
   !    exit
   !end if

   !if(i==1)then
   !write(20,500)px,py,rx,ry, E(t),(t/L_T)!test
   !end if
end do
...
do i=1,lgth_orb_dis
    !orb_dis_lbl=orb_dis_lb+i*orb_dis_mwidth!+0.5*orb_dis_mwidth
    write(26,*)i,orb_labl_dis(i)
end do

!do i=1,len_Y
!    do j=1,len_X
!    write(18,*)Spec_grdX(j),Spec_grdY(i),Spec(i,j)
!    end do
!end do
!-----------------------------------------------------------------------------------------------
!********check initial transverse velocity(2)*********
!py_0(thrd_num*sn+i)=py
!call sta_ana_1D_number(py_0,gn,py_n_v,py_lb,py_ub,py_A,py_sta,py_sta_n)
!py_A(1:py_n_v),py_sta(1:py_n_v),py_sta_n(1:py_n_v)
!do i=1,py_n_v
!write(21,*)py_A(i),py_sta(i),py_sta_n(i)
!end do
!*******************************
...
!H_ph=0.5*(px**2+py**2)+2*((e_N-1)*(1-omega_u(rx,ry))-Z_N)/sqrt(rx**2+(ry-0.5*n_R)**2)-n_Z2/sqrt(rx**2+(ry+0.5*n_R)**2)-(rx*partialt_u1x(rx,ry,t)+ry*partialt_u2x(rx,ry,t))

!else if((rr2<R_M2).and.(rr1>R_M1))then
    
!H_ph=0.5*(px**2+py**2)+2*((e_N-1)*(1-omega_l(rx,ry))-Z_N)/sqrt(rx**2+(ry+0.5*n_R)**2)-n_Z1/sqrt(rx**2+(ry-0.5*n_R)**2)-(rx*partialt_l1x(rx,ry,t)+ry*partialt_l2x(rx,ry,t))

!else

!V_r=-n_Z1/sqrt((rx-0.5*n_R*cos(apha))**2.0+(ry-0.5*n_R*sin(apha))**2.0++ne1**2)-n_Z2/sqrt((rx+0.5*n_R*cos(apha))**2.0+(ry+0.5*n_R*sin(apha))**2.0++ne2**2)
V_r=-n_Z1/sqrt((rx-0.5*n_R*cos(apha))**2+(ry-0.5*n_R*sin(apha))**2+ne1**2)-n_Z2/sqrt((rx+0.5*n_R*cos(apha))**2+(ry+0.5*n_R*sin(apha))**2+ne2**2)!notice that the phase calculation formula has been revised! nucleus charge has been multiplied by 2!!original:V_r=-n_Z/sqrt(rx**2+ry**2+ne**2)
grd_rx=partialt1(rx,ry,t,n_Z1,n_Z2,n_R,apha_II)!partialt1(rx,ry,t,n_Z1,n_Z2,n_R,apha_II)!n_Z1*((rx-0.5*n_R*cos(apha))**2+(ry-0.5*n_R*sin(apha))**2+ne1**2)**(-1.5)*(rx-0.5*n_R*cos(apha))+n_Z2*((rx+0.5*n_R*cos(apha))**2+(ry+0.5*n_R*sin(apha))**2+ne1**2)**(-1.5)*(rx+0.5*n_R*cos(apha))
grd_ry=partialt2(rx,ry,t,n_Z1,n_Z2,n_R,apha_II)!n_Z1*((rx-0.5*n_R*cos(apha))**2+(ry-0.5*n_R*sin(apha))**2+ne1**2)**(-1.5)*(ry-0.5*n_R*sin(apha))+n_Z2*((rx+0.5*n_R*cos(apha))**2+(ry+0.5*n_R*sin(apha))**2+ne1**2)**(-1.5)*(ry+0.5*n_R*sin(apha))
...
implicit none
integer::n_v !<µ÷ÕûÉú³ÉÍ¼ÏñµÄÎïÀí·Ö±æÂÊn_v><Í³¼Æ½á¹ûÊý×éµÄÎ¬Êý>
integer::mn_v !<µ÷ÕûÉú³ÉÍ¼ÏñµÄÏ¸·ÖÎïÀí·Ö±æÂÊmn_v>
integer::len_spec
integer::i,ci,cj,V_i !<Ñ­»·±äÁ¿ºÍÊý×é±êºÅ>
real*8::width !<Í¼ÏñÏñËØµãµÄ¿í¶È>
real*8::mwidth !<Í¼ÏñÏ¸·ÖÏñËØµãµÄ¿í¶È>
real*8::A_exp,W_exp
real*8::v_lb,v_ub !<µ÷ÕûÍ¼Ïñ»æÖÆµÄÇøÓòºÍ·¶Î§>
 ...
 mwidth=width/mn_v
   ...
   if(((A_exp-v_lb)>=0).and.((A_exp-v_ub)<=0)) then
       V_i=(int((A_exp-v_lb)/width))*mn_v
        ...
        do i=1,mn_v                                                   !Í³¼Æ¼ÆËãÊµÑéÊý¾Ý·Ö²¼£¬Êý×éC_sta×ö¼ÇÂ¼
